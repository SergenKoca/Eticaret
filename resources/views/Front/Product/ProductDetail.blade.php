        ...
        <div class="row">
            <div class="col-lg-6">
                <div class="product-pic-zoom">
                    <img class="product-big-img" src="{{asset('images/news/' . \App\Models\Product\Productv2::find($selectProduct->id)->img_url)}}" alt="">
                </div>
                <div class="product-thumbs" tabindex="1" style="overflow: hidden; outline: none;">
                    <div class="product-thumbs-track">
                        <!--<div class="pt active" data-imgbigurl="{{asset('design_front/img/single-product/1.jpg')}}"><img src="{{asset('design_front/img/single-product/thumb-1.jpg')}}" alt=""></div>
                        <div class="pt" data-imgbigurl="{{asset('design_front/img/single-product/2.jpg')}}"><img src="{{asset('design_front/img/single-product/thumb-2.jpg')}}" alt=""></div>
                        <div class="pt" data-imgbigurl="{{asset('design_front/img/single-product/3.jpg')}}"><img src="{{asset('design_front/img/single-product/thumb-3.jpg')}}" alt=""></div>
                        <div class="pt" data-imgbigurl="{{asset('design_front/img/single-product/4.jpg')}}"><img src="{{asset('design_front/img/single-product/thumb-4.jpg')}}" alt=""></div>
                    -->
                    </div>
                </div>
            </div>
            <div class="col-lg-6 product-details">
                <h2 class="p-title">{{$selectProduct->title}}</h2>
                ...
                <div class="p-review">
                    <h5 ><small>@if($view !=0)
                                {{'Bu ürünü'}} {{$view}} {{'defa görüntülediniz'}}
                            @endif</small>
                    </h5>
                </div>
                @endauth
                <form action="{{route('name_front.productDetail.addtoBasket',['product_id'=>$selectProduct->id])}}" method="post">
                    {{csrf_field()}}
                    <div >
                        @foreach($mainProperties as  $key => $ozellik)
                            <li><small>{{$ozellik->title}}</small></li>
                            @foreach($subProperties as $sub)
                                @if($sub->main_property_id == $ozellik->id)
                                    ...
                                    @foreach($productProperties as $item)
                                        @if($item->sub_property_id == $sub->id)
                                            <?php
                                            $b = true;
                                            ?>
                                        @endif
                                    ...
                                    @if($b == true)
                                        <ul class="sub-menu">
                                            <li>
                                                <input id="radioBtnProperty" name="{{$ozellik->id}}" value="{{'subid_'.$sub->id}}" type="radio"> <small>{{$sub->title}}</small>
                                            </li>
                                        </ul>
                                    @else
                                        <ul class="sub-menu">
                                            <li>
                                                <input disabled="true" name="sub_id" value="{{$sub->id}}" type="radio"> <small>{{$sub->title}}</small>
                                            </li>
                                        </ul>
                                    @endif
                                @endif
                        ...
                        @if($price=="-")
                            <input disabled="true" class="site-btn" value="SEPETE EKLE"></input>
                        @else
                            <input id="btnAddToBasket" type="submit" class="site-btn" value="SEPETE EKLE"></input>
                        @endif
                    </div>
                </form>
...
<section class="related-product-section">
    <div class="container">
        ...
        <div class="product-slider owl-carousel">
            @foreach($relatedProductsPrice as $key => $item)
            <div class="product-item">
                    <div class="pi-pic">
                        <img src="{{asset('images/news/' . \App\Models\Product\Productv2::find($relatedProductsInfo[$key]->id)->img_url)}}" alt="">
                        <div class="pi-links">
                            <a href="{{route('name.front.productDetail.index',['id'=>$relatedProductsInfo[$key]->id,'price'=>$relatedProductsPrice[$key],'main_category_id'=>$main_category_id,
                                'sub_category_1_id'=>$sub_category_1_id,'sub_category_2_id'=>$sub_category_2_id])}}" class="add-card"><i class="flaticon-bag"></i><span>ÜRÜNÜ İNCELE</span></a>
                            <a href="#" class="wishlist-btn"><i class="flaticon-heart"></i></a>
                        </div>
                    </div>
                    <div class="pi-text">
                        @if($relatedProductsPrice[$key]=="-")
                            <h6>{{"-"}} {{'₺'}}</h6>
                        @else
                            <h6>{{$relatedProductsPrice[$key]->price}} {{'₺'}}</h6>
                        @endif
                        <p>{{$relatedProductsInfo[$key]->title}}</p>
                    </div>
